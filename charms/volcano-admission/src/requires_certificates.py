...
class CertificatesRequires(Object):
    ...
    def __init__(self, charm, endpoint="certificates", expect_server_certs=False):
        super().__init__(charm, f"relation-{endpoint}")
        self.endpoint = endpoint
        self._expect_server_certs = expect_server_certs
    ...
    def client_certs(self) -> List[Certificate]:
        ...
        return [
            Certificate("client", common_name, **cert) for common_name, cert in certs_data.items()
        ]
    ...
    def server_certs(self) -> List[Certificate]:
        ...
        """
        common_name = self.relation.data[self.charm.unit].get("common_name")
        ...
        certs_data = getattr(self._data, field, {})
        return certs + [
            Certificate("server", common_name, **cert) for common_name, cert in certs_data.items()
        ]
