...
def test_resources(harness, manifests):
    ...
    assert first.metadata.namespace == None
...
def test_apply(lightkube_client, manifests):
    ...
    first, = calls[0].args
    last, = calls[-1].args
    assert (first.kind, first.metadata.name) == ("CustomResourceDefinition", "commands.bus.volcano.sh")
...
def test_successful_delete_resources(manifests, caplog):
    ...
    assert last == "Deleted Service(volcano-scheduler-service, namespace=test_successful_delete_resources)"
...
def test_unexpected_delete_resources(lightkube_client, manifests, caplog):
    ...
    assert exception == "ApiError encountered while attempting to delete resource: Mock Simple Failure"
